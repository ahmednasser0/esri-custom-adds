...
STANDARD_HOURS      = 8.0
PARTTIME_HOURS      = 24.0     # required weekly hours for part-time employees
ANNUAL_LEAVE_NAME   = 'Annual'
...
class Deduction(models.Model):
    ...
    is_absent = fields.Boolean(string='Approved')

    # ─── Part-time weekly deduction (stored by cron) ──────────────────────────

    weekly_deduction_hours = fields.Float(
        string='Weekly Deduction Hours',
        default=0.0,
        help="Part-time weekly deduction: set by the weekly cron when the "
             "employee worked less than 24 hours in the week (top-3 days capped at 8h).",
    )
    ...
    @api.depends('name', 'date', 'weekly_deduction_hours')
    def _compute_day_values(self):
        for rec in self:
            if rec.name and rec.date:
                ...
                # Weekly part-time deduction overrides the daily deduction
                if rec.weekly_deduction_hours > 0:
                    ded = rec.weekly_deduction_hours
    ...
    # ─── Part-time weekly deduction cron ─────────────────────────────────────

    @api.model
    def _cron_create_parttime_deductions(self, week_sunday=None):
        """
        Run weekly (on Friday) to calculate part-time employee deductions.

        Logic:
        - Find employees whose resource_calendar_id.name contains 'Part Time'.
        - For the Sun–Thu week, collect each day's worked hours capped at 8h.
        - Take the top-3 days by capped hours.
        - If their sum < 24h  →  deduction = 24 - sum, stored on a deduction
          record dated Thursday of that week.
        """
        today = fields.Date.today()

        if week_sunday is None:
            # Python weekday: Mon=0 … Sun=6
            # We want the most recent Sunday
            days_back = (today.weekday() + 1) % 7  # days since last Sunday
            week_sunday = today - timedelta(days=days_back)

        week_thursday = week_sunday + timedelta(days=4)

        part_time_employees = self.env['hr.employee'].search([
            ('active', '=', True),
            ('resource_calendar_id.name', 'ilike', 'Part Time'),
        ])

        for employee in part_time_employees:
            attendances = self.env['hr.attendance'].search([
                ('employee_id',     '=', employee.id),
                ('attendance_date', '>=', week_sunday),
                ('attendance_date', '<=', week_thursday),
            ])

            # Sum worked hours per day, capped at 8h
            day_hours = {}
            for att in attendances:
                if att.day_name in WORKING_DAYS:
                    capped = min(att.worked_hours, STANDARD_HOURS)
                    day_hours[att.attendance_date] = day_hours.get(att.attendance_date, 0.0) + capped

            # Top-3 days
            top3_total = sum(sorted(day_hours.values(), reverse=True)[:3])
            deduction  = max(PARTTIME_HOURS - top3_total, 0.0)
            ...
            if existing:
                existing.write({'weekly_deduction_hours': deduction})
            else:
                self.create({
                    'name':                   employee.id,
                    'date':                   week_thursday,
                    'weekly_deduction_hours': deduction,
                })